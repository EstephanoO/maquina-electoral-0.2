...
const ProgressChart = dynamic(
  () => import("@/modules/dashboards/EventTierraCharts").then((mod) => mod.ProgressChart),
  {
    ssr: false,
...
const TimelineChart = dynamic(
  () => import("@/modules/dashboards/EventTierraCharts").then((mod) => mod.TimelineChart),
  {
    ssr: false,
    loading: () => <div className="h-full w-full rounded-md bg-muted/40" />,
  },
...
const PaceChart = dynamic(
  () => import("@/modules/dashboards/EventTierraCharts").then((mod) => mod.PaceChart),
  {
    ssr: false,
    loading: () => <div className="h-full w-full rounded-md bg-muted/40" />,
  },
            ...
            <div className="flex flex-wrap items-center gap-2">
              {candidateMeta.map((candidate) => (
                <Button
                  ...
                  variant="outline"
                  asChild
                  className="h-8 rounded-md border-border/60 bg-background/70 px-3 text-xs"
                >
                  <Link href={`/dashboard/${candidate.slug}/tierra`}>{candidate.name}</Link>
                </Button>
              ))}
              <Button
                size="sm"
                ...
                className="h-8 rounded-md border-border/60 bg-background/70 px-3 text-xs"
              >
                <Link href="/eventos/28-enero/dashboard">Dashboard general</Link>
              </Button>
            </div>
          </div>
        </Card>
