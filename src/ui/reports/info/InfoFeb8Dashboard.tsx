...
const CSV_PATH = "/registros_2026-02-09-_1_ (1).csv";
...
const parseCsv = (payload: string): InterviewRecord[] => {
  return payload
    .split(/\r?\n/)
    .map((line) => line.trim())
...
export default function InfoFeb8Dashboard() {
  ...
  const fetchRecords = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(encodeURI(CSV_PATH), { cache: "no-store" });
  ...
  React.useEffect(() => {
    ...
    const stored = window.localStorage.getItem("info-feb-8-status");
    if (stored) {
      try {
        const parsed = JSON.parse(stored) as Record<string, RecordStatus>;
        setStatusMap(parsed ?? {});
      } catch {
        setStatusMap({});
      }
  ...
  React.useEffect(() => {
    ...
    window.localStorage.setItem("info-feb-8-status", JSON.stringify(statusMap));
    setSavePulse(true);
  ...
  const setRecordStatus = React.useCallback(
    (phone: string, next: Partial<RecordStatus>) => {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(22,57,96,0.16),_transparent_55%),radial-gradient(circle_at_bottom,_rgba(255,200,0,0.12),_transparent_55%)] text-foreground">
      ...
      <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-6 px-4 py-6 md:px-6">
        <section className="sticky top-4 z-20 rounded-3xl border border-border/70 bg-card/80 px-4 py-4 shadow-lg shadow-black/5 backdrop-blur md:px-6">
                    ...
                    <TableBody>
                      {activeConversations.length === 0 ? (
                        ...
                        activeConversations.map((record) => (
                          <TableRow key={`active-${record.id}-${record.phone}`}>
